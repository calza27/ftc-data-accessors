...
func main() {
	lambda.Start(getFireteamListHandler)
}

func getFireteamListHandler(ctx context.Context, request events.APIGatewayProxyRequest) (events.APIGatewayProxyResponse, error) {
	...
	data, err := s3Service.Get(constants.FIRETEAMS_FILE)
	if err != nil {
		errMsg := fmt.Sprintf("Error getting fireteams file contents from S3: %s", err.Error())
		log.Print(errMsg)
